...
class IPTypeRule(AnsibleLintRule):
    ...
    description = 'IP address must be in valid format. See https://docs.ansible.com/ansible/latest/collections/community/general/ufw_module.html for more details'
    tag = 'Formatting'
    ...
    def matchtask(
        ...
        if task["action"]["__ansible_module__"] in self._modules:
            ip = task['action'].get('from_ip', None)
